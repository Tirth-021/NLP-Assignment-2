...
SAE_DIR = "./sae_models"
DIR_FILE = "./sae_artifacts/directions.pt"
NUM_HEADS = 32
HIDDEN = 3072
HEAD_DIM = HIDDEN // NUM_HEADS
ALPHA = 1.2   # steering strength — adjust later
...
def load_sae_pt(path):
    state = torch.load(path, map_location="cpu")
    d_latent, d_model = state["encoder.weight"].shape
    sae = SAE(d_model, d_latent)
    sae.load_state_dict(state)
    sae.half().to(CONFIG["device"])
    return sae.encoder.weight.data, sae.decoder.weight.data  # (latent, d_model), (d_model, latent)

# Load all SAEs into dictionary
SAE_BANK = {}
for f in os.listdir(SAE_DIR):
    if f.endswith("_sae.pt"):
        parts = f.replace(".pt","").split("_")
        layer = int(parts[0].replace("L",""))
        head = int(parts[1].replace("H",""))
        E, D = load_sae_pt(os.path.join(SAE_DIR,f))
        SAE_BANK[(layer, head)] = (E, D)

DIRECTIONS = torch.load(DIR_FILE, map_location="cpu")

# alpha steering strength (fixed or adaptive)
alpha = torch.full((32, NUM_HEADS),
                   0.8, device=CONFIG["device"])  # <--- you said keep alpha = 0.8

def make_v_steering_hook(layer):
    def hook(module, inp, out):
        B, S, threeH = out.shape
        q, k, v = out.split(threeH // 3, dim=-1)

        # reshape V → (batch*seq, heads, head_dim)
        v = v.view(B * S, NUM_HEADS, HEAD_DIM)

        for h in range(NUM_HEADS):
            key = (layer, h)
            if key not in SAE_BANK or key not in DIRECTIONS:
                continue

            E, D = SAE_BANK[key]        # (latent, 96), (96, latent)
            direction = DIRECTIONS[key]  # (latent,)

            # match datatypes + device
            v_h = v[:, h, :].to(v.dtype)
            E = E.to(v_h.device, dtype=v_h.dtype)
            D = D.to(v_h.device, dtype=v_h.dtype)
            direction = direction.to(v_h.device, dtype=v_h.dtype)

            direction = direction / (direction.norm() + 1e-12)

            # Encode to latent representation
            z = F.relu(v_h @ E.t())   # (BS, latent)

            # -------------------------------
            # Directional Safety Steering
            # proj = (z @ direction)    
            # z = z - ALPHA * proj.unsqueeze(-1) * direction
            z = z - ALPHA * direction
            # -------------------------------

            # Decode back to V-space
            v[:, h, :] = z @ D.t()

        # reshape V back to original shape
        v = v.view(B, S, -1)
        return torch.cat([q, k, v], dim=-1)
    return hook

def attach_sae_hooks(model):
    for L in range(model.config.num_hidden_layers):
        model.model.layers[L].self_attn.qkv_proj.register_forward_hook(make_v_steering_hook(L))
    return model
...
def main():
    ...
    safe_model = attach_sae_hooks(base_model_for_wrapper)
    ...
    mitigated_results, mitigated_stats = generate_and_evaluate(
        safe_model, base_tokenizer, dataset_samples, 
        "SAE-Safe (Phi-3)", perspective_client
    )
    del safe_model, base_model_for_wrapper 
    clear_gpu_memory()
