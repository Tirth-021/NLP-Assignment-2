...
TARGET_LAYERS = [10, 12, 14, 17, 20, 23] 

n_heads = base_model.config.num_attention_heads
head_dim = base_model.config.hidden_size // n_heads

SAVE_DIR = "acts_sae"
os.makedirs(SAVE_DIR, exist_ok=True)
print(f"Saving per-head V activations to: {SAVE_DIR}")
...
# =====================================================================
# COLLECT VALUE PROJECTION ACTIVATIONS FOR SAE TRAINING
# =====================================================================

# Storage buffers: per-layer → {"safe": [], "harm": []}
storage = {L: {"safe": [], "harm": []} for L in TARGET_LAYERS}

def make_v_hook(layer_idx, tag):
    def hook(module, inp, out):
        # out: [B, S, 3*H]
        B, S, threeH = out.shape
        H = threeH // 3
        _, _, V = out.split(H, dim=-1)       # take the Value part only
        V = V.view(B*S, n_heads, head_dim)   # reshape per-head
        storage[layer_idx][tag].append(V.detach().cpu().numpy())
    return hook
...
for L in TARGET_LAYERS:
    mod = base_model.model.layers[L].self_attn.qkv_proj
    hooks.append(mod.register_forward_hook(make_v_hook(L, "safe")))

print("\nCollecting SAFE V activations...")
for batch in tqdm(safe_loader):
...
# --------------------------
# Pass HARMFUL examples
# --------------------------
hooks = []
for L in TARGET_LAYERS:
    mod = base_model.model.layers[L].self_attn.qkv_proj
    hooks.append(mod.register_forward_hook(make_v_hook(L, "harm")))

print("\nCollecting HARMFUL V activations...")
for batch in tqdm(harm_loader):
    ids = batch["input_ids"].to(DEVICE)
    mask = batch["attention_mask"].to(DEVICE)
    with torch.no_grad():
        _ = base_model(input_ids=ids, attention_mask=mask)
...
# --------------------------
# Save to disk per-layer, per-head
# --------------------------
print("\nSaving activation tensors ...")
for L in TARGET_LAYERS:
    safe_arr = np.concatenate(storage[L]["safe"], axis=0)   # (N, heads, head_dim)
    harm_arr = np.concatenate(storage[L]["harm"], axis=0)

    for h in range(n_heads):
        np.save(f"{SAVE_DIR}/L{L}_H{h}_safe.npy", safe_arr[:, h, :])
        np.save(f"{SAVE_DIR}/L{L}_H{h}_harm.npy", harm_arr[:, h, :])
